...
void sim_reset(void)
{
    ...
    const float limit = BOX_CORNER_R - WALL_MARGIN - 8.0f;
    ...
    s_count = PARTICLE_COUNT;
    int i = 0;
    int layers = 0;
    for (int layer = 0; i < s_count; layer++) {
        const float along = limit - spacing * 0.5f - (float)layer * spacing;
        if (along < 0.0f) {
            break;
        }
        layers = layer + 1;
        for (int iz = 0; iz < nz && i < s_count; iz++) {
            for (int k = -nx; k <= nx && i < s_count; k++) {
                const float x = cx + ux * along + px * (float)k * spacing +
                                (rand_unit() - 0.5f) * spacing * 0.2f;
                const float yy = cy + uy * along + py * (float)k * spacing +
                                 (rand_unit() - 0.5f) * spacing * 0.2f;
                const float z = z0 + (float)iz * spacing + (rand_unit() - 0.5f) * spacing * 0.2f;
                const float dx = x - cx;
    ...
    if (i < s_count) {
        ESP_LOGW(TAG, "only %d of %d fit inside the circle", i, s_count);
        s_count = i;
    }
    ESP_LOGI(TAG, "seeded %d particles (%dx%d per layer, %d layers)",
             s_count, nx, nz, layers);
    resolve_walls();
}
...
static void resolve_walls(void)
{
    ...
    for (int i = 0; i < s_count; i++) {
        ...
        if (WALL_SPEED_CAP > 0.0f) {
            const float s2 = s_p->vel[i][0] * s_p->vel[i][0] +
                             s_p->vel[i][1] * s_p->vel[i][1] +
                             s_p->vel[i][2] * s_p->vel[i][2];
            const float cap2 = WALL_SPEED_CAP * WALL_SPEED_CAP;
            if (s2 > cap2) {
                const float s = sqrtf(s2);
                const float sc = WALL_SPEED_CAP / s;
                s_p->vel[i][0] *= sc;
                s_p->vel[i][1] *= sc;
                s_p->vel[i][2] *= sc;
            }
...
static void substep(float dt, const sim_forces_t *f)
{
    ...
    if (RISE_CAP > 0.0f) {
        float gx = f->gravity[0], gy = f->gravity[1], gz = f->gravity[2];
        ...
        if (gm > 1e-3f) {
            ...
            gz /= gm;
            for (int i = 0; i < s_count; i++) {
                const float along = s_p->vel[i][0] * gx +
                                    s_p->vel[i][1] * gy +
                                    s_p->vel[i][2] * gz;
                if (along < -RISE_CAP) {
                    const float extra = along + RISE_CAP;
                    s_p->vel[i][0] -= extra * gx;
                    s_p->vel[i][1] -= extra * gy;
                    s_p->vel[i][2] -= extra * gz;
                }
            }
        }
    }
